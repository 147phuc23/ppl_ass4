...
class Emitter():
    ...
    def emitNEWARRAY(self, inType):
        if isinstance(inType, StringType):
            return self.jvm.emitANEWARRAY(self.getFullType(inType))
        return self.jvm.emitNEWARRAY(self.getFullType(inType))
    ...
    *    @param typ the type of the constant
    ...
    def emitALOAD(self, in_, frame):
        ...
        elif type(in_) is ArrayPointerType or type(in_) is ArrayType or type(in_) is StringType:
    ...
    *   @param toLabel the ending label  of the scope where the variable is active.
    ...
    def emitREADVAR(self, name, inType, index, frame):
        ...
        elif type(inType) is ArrayPointerType or type(inType) is ArrayType or type(inType) is StringType:
    ...
    *   @param name the symbol entry of the variable.
    ...
    def emitWRITEVAR(self, name, inType, index, frame):
        ...
        elif type(inType) is ArrayPointerType or type(inType) is StringType or type(inType) is ArrayType:
    ...
    *   generate ior
    ...
    def emitREOPFlOAT(self, op, in_, frame):
        result = []
        label1 = frame.getNewLabel()
        label2 = frame.getNewLabel()
        ...
        result.append(self.emitRELOP(op, IntType(), str(label1), str(label2),frame))
        return ''.join(result)
    ...
    def emitREOP(self, op, in_, frame):
        ...
        if isinstance(in_, IntType) or isinstance(in_, BoolType):
            if op == ">":
    ...
    def emitRELOP(self, op, in_, trueLabel, falseLabel, frame):
        ...
        result.append(self.emitPUSHCONST("1", IntType(), frame))
        result.append(self.emitGOTO(str(trueLabel), frame))
        frame.pop()
        result.append(self.emitLABEL(str(falseLabel), frame))
        result.append(self.emitPUSHCONST("0", IntType(), frame))
        result.append(self.emitLABEL(str(trueLabel), frame))
        return ''.join(result)
    ...
    *   @param in the type of the returned expression.
    ...
    def emitRETURN(self, in_, frame):
        ...
        elif type(in_) is StringType or type(in_) is ArrayPointerType:
            frame.pop()
